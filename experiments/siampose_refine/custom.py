from models.siampose_sharp import SiamMask
...
class MaskCorr(Mask):
    def __init__(self, oSz=63):
        ...
        self.oSz = oSz
        self.mask = DepthCorr(256, 256, self.oSz**2*17)
...
class Refine(nn.Module):
    def __init__(self):
        ...
        #
        # self.h1 = nn.Sequential(nn.Conv2d(128, 64, 3, padding=1), nn.ReLU())
        #
        # self.h0 = nn.Sequential(nn.Conv2d(64, 32, 3, padding=1), nn.ReLU())
        #
        # self.deconv = nn.ConvTranspose2d(256, 256, 15, 15)
        #
        # self.post0 = nn.Conv2d(128, 128, 3, padding=1)
        # self.post1 = nn.Conv2d(64, 64, 3, padding=1)
        # self.post2 = nn.Conv2d(32, 17, 3, padding=1)
        self.deconv = nn.ConvTranspose2d(256, 256, 15, 15)
        self.deconv1 = nn.ConvTranspose2d(256, 256, 4, stride=2)
        self.deconv2 = nn.ConvTranspose2d(256, 256, 4, stride=2)
        self.conv = nn.Conv2d(256, 17, 1)
        self.up_sample = nn.UpsamplingBilinear2d(size=[127, 127])
    ...
    def forward(self, f, corr_feature, pos=None, test=False):
        ...
        # out = self.deconv(p3)
        # out = self.post0(F.upsample(self.h2(out), size=(31, 31)))
        # out = self.post1(F.upsample(self.h1(out), size=(61, 61)))
        # out = self.post2(F.upsample(self.h0(out), size=(127, 127)))
