...
vis_outpath = './vis_outpose/'

if not isdir(vis_outpath): mkdir(vis_outpath)
...
def select_gt_img(mask, weight, g_sz=127):
    ...
    mask_uf = torch.transpose(mask_uf, 1, 2).contiguous().view(-1, g_sz * g_sz * 3)
    mask_uf = torch.index_select(mask_uf, 0, pos)
    mask_uf = mask_uf.view(-1, 3, g_sz, g_sz)
